...
public partial class PreferencesWindow : Window
{
    ...
    public PreferencesWindow()
    {
        ...
        Opened += (_, _) =>
        {
            Services.ThemeManager.UseMacNativeChrome(this, PrefsTitleBar);
            WinChrome.RoundCorners(this, true);
            if (Content is Control root) Motion.ScaleIn(root, 0.96, 180);
        };
        SmoothScroll.Attach(PrefsScroll);
        ...
        ExportBtn.Click += async (_, _) =>
        {
            ...
            var snapshot = vm.SnapshotExport();
        ...
        AboutUpdateNote.Text = canUpdate
            ? (OperatingSystem.IsMacOS()
                ? "Updates download inside the app, so macOS does not ask you to approve them the way it "
                  + "did the first install. Your notebooks are never touched."
                : "This is a portable build: updating replaces the program files and leaves your userdata "
                  + "folder alone.")
            : "This copy can't update itself in place. It's running from a development tree, or from a "
    ...
    private void WirePaletteEditor(WrapPanel chips, TextBox hexBox, Button reset, bool highlight)
    {
        hexBox.KeyDown += (_, e) =>
        {
            ...
            if (highlight && hex.Length == 7) hex = "#66" + hex[1..];
    ...
    {
        if (_capturing is { } prev) EndShortcutCapture(prev.Btn, prev.Action);
    ...
    private void RefreshFontChoices()
    {
        ...
        var choices = AppFonts.ListNames(vm.ExtendedFonts)
