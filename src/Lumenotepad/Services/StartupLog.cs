...
public static class StartupLog
{
    ...
    public static void Mark(string stage)
    {
        try
        {
            lock (Gate)
            {
                ...
                File.AppendAllText(_path, $"{DateTime.Now:HH:mm:ss.fff}  {stage}\n");
            }
        }
        catch
        {
    ...
    public static void Fail(string stage, Exception ex) => Mark($"{stage} FAILED: {ex.GetType().Name}: {ex.Message}");
}
